...
For single-core images, the selected DTS is generated by `nemu_board` at build
...
Device-tree templates are generated at build time through
`scripts/generate-nemu-board-dts.py` and `nemu_board/dts/DTSGen.py` into
`build/generated-dts`; `DEFAULT_DTB=device_name` selects the generator profile.
