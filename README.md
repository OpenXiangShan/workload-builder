...
`nemu_defconfig` is the XiangShan/NEMU tree. NutShell is RV64IMAC; Linux
images use `nutshell_defconfig` (lp64) when `DEFAULT_DTB` is `nutshell`,
...
The canonical DTS generator usage, DRAM profiles, and DTS selection examples
are in [dts/README.md](dts/README.md).

For single-core images, the selected DTS is generated by the bundled generator at build
time and is the source of truth for the DRAM
