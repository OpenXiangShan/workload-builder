...
# Shared by nutshell / nutshell-mem2g / nutshell-fpga. Omit Zicntr/Zihpm:
# U-mode cycle/time/instret are not mapped, and HPM is not architectural.
NUTSHELL_ISA_EXTENSIONS = ["i", "m", "a", "c", "zicsr", "zifencei"]
NUTSHELL_LEGACY_ISA = "rv64imac_zicsr_zifencei"
...
def parse_name(name):
    ...
    fixed = {
        "xiangshan": (1, 128 << 20),
        "nutshell": (1, 128 << 20),
        "nutshell-mem2g": (1, 2 << 30),
        "nutshell-fpga": (1, 2 << 30),
        "spike": (1, 2 << 30),
        "spike-2core": (2, 2 << 30),
        "yanqihu": (1, 128 << 20),
...
def render_nutshell_fpga(DTSGen, memory):
    """NutShell FPGA: UART16550 + PLIC, 2 GiB, rv64imac/Sv39."""
    ...
    gen = DTSGen(
        compatible="freechips,rocketchip-unknown-dev",
        model="ucas,nutshell-fpga",
        cpu_compatibles=["UCAS,COOSCA1.0", "riscv"],
        isa_extensions=DTSGen.sort_isa_extensions(NUTSHELL_ISA_EXTENSIONS),
        legacy_isa=NUTSHELL_LEGACY_ISA,
        mmu_type="riscv,sv39",
        timebase_freq=1000000,
        memories=[(DRAM_BASE, memory)],
        reserved_memories=[],
        uartlite_addr=None,
        plic_addr=0x3c000000,
        plic_ndev=66,
        cpu_properties=cpu_props,
        ...
        plic_node_name="interrupt-controller",
...
def render(name, isa_config_name=None):
    ...
    elif name == "nutshell-fpga":
        gen = render_nutshell_fpga(DTSGen, memory)
    elif name in {"nutshell", "nutshell-mem2g"}:
        gen = DTSGen(compatible="freechips,rocketchip-unknown-dev",
                     model="xiangshan,xiangshan-kunminghu",
                     cpu_compatibles=["UCAS,COOSCA1.0", "riscv"],
                     isa_extensions=DTSGen.sort_isa_extensions(NUTSHELL_ISA_EXTENSIONS),
                     legacy_isa=NUTSHELL_LEGACY_ISA, mmu_type="riscv,sv39", timebase_freq=1000000,
                     clint_addr=0x38000000, plic_addr=None, memories=[(DRAM_BASE, memory)],
