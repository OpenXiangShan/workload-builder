...
INITRAMFS_OFFSET_ALIGNMENT_MB="${INITRAMFS_OFFSET_ALIGNMENT_MB:-1}"
HARTS="${HARTS:-2}"
...
case "$INITRAMFS_OFFSET_ALIGNMENT_MB" in
    ''|*[!0-9]*) echo "INITRAMFS_OFFSET_ALIGNMENT_MB must be a positive integer" >&2; exit 1 ;;
esac
if [ "$INITRAMFS_OFFSET_ALIGNMENT_MB" -eq 0 ]; then
    echo "INITRAMFS_OFFSET_ALIGNMENT_MB must be a positive integer" >&2
    exit 1
fi
INITRAMFS_BEGIN_ADDR=$((
    (INITRAMFS_BEGIN_ADDR + INITRAMFS_OFFSET_ALIGNMENT_MB * MEGABYTE - 1) /
    (INITRAMFS_OFFSET_ALIGNMENT_MB * MEGABYTE) *
    (INITRAMFS_OFFSET_ALIGNMENT_MB * MEGABYTE)
))
