...
linux/$(1): $(if $(filter 1,$(VIRTUALIZATION)),,$(SPEC2017_BUILD_DIR)/$(1)/$(SPEC2017_FIRMWARE_FILENAME))
...
linux/spec2017: spec2017-check-spec-config
	...
	@if [ "$(VIRTUALIZATION)" = 1 ]; then \
		$(MAKE) --no-print-directory VIRTUALIZATION=1 BENCH="$(BENCH)" INPUT="$(INPUT)" MODE="$(MODE)" linux/$(SPEC2017_CASE); \
	else \
