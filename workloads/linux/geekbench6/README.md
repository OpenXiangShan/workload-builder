...
Build Geekbench 6 as a nested KVM guest and run the Host firmware with QEMU:

```sh
make linux/geekbench6 PLATFORM=qemu -jN
make virt bin=build/linux-workloads/geekbench6/fw_payload.qemu.bin
QEMU_BIN=/path/to/qemu-system-riscv64 QEMU_MEMORY=16G \
  bash scripts/run-qemu.sh \
  build/virt/geekbench6/host/fw_payload.qemu.bin
```
